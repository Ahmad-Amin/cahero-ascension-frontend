    ...
    <div className="p-8 bg-transparent min-h-screen">
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan, index) => (
          <div
            key={index}
            className="bg-[#000000] border border-[#dbaa24] rounded-xl p-6 space-y-7 text-white relative"
          >
            <div className="flex justify-between items-center">
              <div className="flex flex-row gap-3 flex-1">
                <h3 className="text-xl font-semibold text-[#dbaa24]">
                  {plan.name}
                </h3>
              </div>
            ...
            <ul className="space-y-5 mb-6">
              {plan.features.map((feature, i) => (
                <li key={i} className="flex items-center text-lg">
                  <span className="text-[#dbaa24] mr-2">●</span>
                  {feature}
                </li>
              ))}
            </ul>
            <div className="w-full flex justify-center">
              <button className="bg-[#dbaa24] w-1/2 py-3 rounded-md text-lg mt-5 font-semibold hover:bg-[#a7821b] transition ease-in-out duration-300">
                View
              </button>
            </div>
          </div>
